...
export default function Library() {
  ...
  const fileUrl = useCallback((p: string | undefined, { isGallery = false } = {}) => {
    const raw = String(p || "").trim();
    if (!raw) return "";
    if (raw.startsWith("http")) return raw;
    let path = raw.startsWith("/") ? raw : `/${raw}`;
    if (isGallery && !path.startsWith("/uploads/")) {
  ...
  const downloadBookUrl = (id: string | number) => `${apiRoot}/api/books/${id}/download`;
  const downloadTreeUrl = (id: string | number) => `${apiRoot}/api/trees/${id}/gedcom`;
  const isMockTree = (tree: any) => !Number.isFinite(Number(tree?.id));
  ...
  const handleViewTree = async (tree: any) => {
    ...
    try {
      ...
      setViewPeople([]);
      setViewTreeError(getApiErrorMessage(err, t("tree_builder_error", "Failed to load tree.")));
    } finally {
  ...
  const selectedImageUrl = (selectedImage?.image_path ?? selectedImage?.imagePath)
    ? fileUrl(selectedImage.image_path ?? selectedImage.imagePath, { isGallery: true })
    : "";
      ...
      hero={
        <div className="space-y-4">
          <SEO
            ...
            description="Explore our curation of books, manuscripts, and original research documents detailing Maghrebi lineage records and heritage history."
            keywords={["Maghreb library books", "Maghrebi genealogy books", "Maghreb history library"]}
          />
          ...
          >
            <div className="relative">
              ...
              <input
                ...
                className={`library-search-input w-full pl-10 py-3 rounded-md bg-transparent border ${borderColor} outline-none ${theme === "dark" ? "text-white" : "text-[#2c1810]"
                  }`}
              />
            </div>
                    ...
                    <div className="p-5 space-y-4">
                      ...
                      <div className="text-xs opacity-70 flex items-center gap-2 flex-wrap">
                        ...
                        {tree.hasGedcom && tree.data_format === "gedcomx" ? (
                          <span className="px-2 py-0.5 rounded bg-[#5d4037]/20 text-[#5d4037] font-medium">
                        ...
                        {tree.hasGedcom && tree.data_format !== "gedcomx" && tree.data_format !== "gedcom7" ? (
                          <span className="px-2 py-0.5 rounded bg-[#5d4037]/10 text-[#5d4037]/80 font-medium">
          ...
          {filteredGallery.length === 0 ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredGallery.map((item) => {
                const imageSrc = (item.image_path ?? item.imagePath) ? fileUrl(item.image_path ?? item.imagePath, { isGallery: true }) : "";
                return (
                  <div
                    key={item.id}
            ...
            <div className="flex-1 relative bg-black/5 overflow-hidden">
              {viewLoading ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center p-6">
                  <div className="rounded-lg border border-[#d8c7b0] bg-white/90 dark:bg-[#2c1810] dark:border-[#5d4037] px-6 py-5 text-sm text-[#5d4037] dark:text-[#e8dfca] shadow-xl text-center max-w-md">
