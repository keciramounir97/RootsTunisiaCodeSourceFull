...
export default function Books() {
  ...
  const validateBookFile = (file: File) => {
    ...
    if (ext && ext !== undefined && !allowedBookExts.has(ext)) {
      return t(
        "invalid_book_type",
  ...
  const openProtected = async (book: any) => {
    if (!book?.id) return;
    setSaving(true);
    setError("");
    try {
  ...
  const downloadProtected = async (book: any) => {
    if (!book?.id) return;
    setSaving(true);
    setError("");
    try {
  ...
  const downloadUrl = (id: any) => `${apiRoot}/api/books/${id}/download`;
